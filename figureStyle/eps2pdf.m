...
%%%%% modified by Sebastian Eicke
if ismac
    fullGsPath = '/usr/local/bin/gs';
end
if ispc
	if strcmp(computer('arch'), 'win64')
		fullGsPath = 'C:\Programme\gs\gs9.05\bin\gswin64c.exe';
	elseif strcmp(computer('arch'), 'win32')
		fullGsPath = 'C:\Programme\gs\gs9.05\bin\gswin32c.exe';
	else
		error('Check Path of gswinXXc.exe');
	end
end
%%%%% end
...
if ~ok
    status = ['Fehler bei der Erstellung der temporären eps Datei: ' epsFile ' - ' errStr];
    if nargout,      result = 1;    end;
    if nargout > 1,  msg = status;  else, disp(status); end;
else
    % Run Ghostscript
    ...
    if stat
        status = ['Ghostscript Fehler - GS Pfad überprüfen: ' result];
        if nargout,      result = 1;    end;
        if nargout > 1,  msg = status;  else, disp(status);  end;  
    else
%        status = 'pdf successfully created'; 
%        status = char(strcat(epsFile, ' erstellt'));
status = '';
